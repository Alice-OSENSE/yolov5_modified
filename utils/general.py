    ...
    for i, d in enumerate(x):  # per image
        if d is not None and len(d):
            ...
            for j, a in enumerate(d):  # per item
                ...
                # cv2.imwrite('utest%i.jpg' % j, cutout)
...
def plot_one_box(x, img, color=None, write_label=True, label=None, line_thickness=None):
    # Plots one bounding box on image img
    tl = line_thickness or round(0.002 * (img.shape[0] + img.shape[1]) / 2) + 1  # line/font thickness
    color = [random.randint(0, 255) for _ in range(3)]
    c1, c2 = (int(x[0]), int(x[1])), (int(x[2]), int(x[3]))
    cv2.rectangle(img, c1, c2, (255, 0, 0), thickness=1, lineType=cv2.LINE_AA)
...
def plot_test_txt():  # from utils.general import *; plot_test()
    # Plot utest.txt histograms
    x = np.loadtxt('utest.txt', dtype=np.float32)
    box = xyxy2xywh(x[:, :4])
    cx, cy = box[:, 0], box[:, 1]
...
def plot_study_txt(f='study.txt', x=None):  # from utils.general import *; plot_study_txt()
    # Plot study.txt generated by utest.py
